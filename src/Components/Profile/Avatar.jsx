...
import axiosInstance from "../../API/axiosConfig"; // ✅ use the configured axios
import BASE_URL from "../../API/api"; // used for profile/update endpoint (can also use axiosInstance)
...
const Avatar = () => {
  ...
  const flatAvatars = useMemo(() => {
    ...
    return avatarTree.flatMap((top) =>
      (top.children ?? []).flatMap((cat) =>
        (cat.children ?? []).flatMap((sub) => sub.avatars ?? [])
      )
    );
  }, [avatarTree]);
          ...
          <div className="row px-2">
            ...
            {avatarsError && (
              <p className="text-danger mt-3">
                {avatarsErr?.message || "Error loading avatars"}
              </p>
            )}
...
function CategorySection({ node, selectedAvatarId, setSelectedAvatarId }) {
  ...
  return (
    <div className="mb-4">
      ...
      {node.category_name && (
        <h5 className="text-white mb-2 text-capitalize">
          {node.category_name}
        </h5>
      )}
        ...
        <div className="d-flex flex-wrap mb-2">
          {node.avatars.map((av) => (
            <div
              key={av.id}
              className={`avatar-container ${
                selectedAvatarId === av.id ? "selected-avatar" : ""
              }`}
              onClick={() => setSelectedAvatarId(av.id)}
            ...
            >
              <img
                ...
                style={{
                  ...
                  objectFit: "cover",
                  outline:
                    selectedAvatarId === av.id ? "3px solid #e74c3c" : "none",
                }}
      ...
      {hasChildren &&
        node.children.map((child) => (
          <div key={child.id} className="ps-2">
            {child.category_name && (
              <h6 className="text-light">{child.category_name}</h6>
            )}
            <CategorySection
              node={child}
              selectedAvatarId={selectedAvatarId}
