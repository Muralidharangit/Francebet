...
const SearchTopGames = () => {
  ...
  // State to control the confirmation modal's visibility
  const [showModal, setShowModal] = useState(false);
  const [iframeLoaded, setIframeLoaded] = useState(false);

  // Navbar back button click handler:
  // Shows the modal, preventing immediate navigation.
  const handleBackClick = () => {
    setShowModal(true);
  };

  // Confirm navigation:
  // Closes the modal and performs the back navigation.
  const handleConfirm = () => {
    setShowModal(false);
              ...
              <div>
                {showFullScreenGame && selectedGameUrl ? (
                  <>
                    {/* Fullscreen Layout */}
                    <div
                      className="position-fixed top-0 start-0 w-100 vh-100 bg-black d-flex flex-column"
                      style={{ zIndex: 9999 }}
                    >
                      {/* Navbar: only show if iframe loaded */}
                      {iframeLoaded && (
                        <nav
                      ...
                      >
                        <iframe
                          ...
                          onLoad={() => setIframeLoaded(true)} // track iframe load
                          onError={() => setIframeLoaded(false)} // hide navbar if load fails
                        />
                      </div>
                    </div>
