...
const SelectAmount = ({ amount, setAmount, token, onValidityChange }) => {
  ...
  const setErrorIfOutOfRange = (num) => {
    ...
    if (limits.min != null && num < limits.min) {
      setError(
        `Minimum allowed is ${CURRENCY_SYMBOL} ${limits.min.toLocaleString(
          "en-IN"
        )}`
      );
      return;
    ...
    if (limits.max != null && num > limits.max) {
      setError(
        `Maximum allowed is ${CURRENCY_SYMBOL} ${limits.max.toLocaleString(
          "en-IN"
        )}`
      );
      return;
          ...
          {showRange && (
            <small className="text-danger d-block mb-3">
              {limits.min != null && limits.max != null ? (
                ...
                <>
                  Minimum: {CURRENCY_SYMBOL}{" "}
                  {limits.min.toLocaleString("en-IN")}
                </>
              ) : limits.max != null ? (
                <>
                  Maximum: {CURRENCY_SYMBOL}{" "}
                  {limits.max.toLocaleString("en-IN")}
                </>
              ) : null}
            </small>
          )}
