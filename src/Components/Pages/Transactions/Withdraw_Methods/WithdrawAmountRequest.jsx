...
const WithdrawAmountRequest = ({ amount, bankId }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setErrors, resetForm }) => {
      try {
        ...
        const response = await sendWithdrawRequestWallet({
          token,
          bankId,
  ...
  useEffect(() => {
    ...
    const fetchBankDetails = async () => {
      try {
        const response = await EditwalletBank(bankId, token);
        if (response.status === "success") {
          ...
          {(bankDetails || bank) && (
            <div className="bank-info bg-glass p-3 rounded-4 mt-3">
              ...
              <div className="text-grey small">
                <p className="mb-1">
                  Name: <strong>{bankDetails?.name || bank?.name}</strong>
                </p>

                <p className="mb-1">
                  Phone Number:{" "}
                  <strong>
                    {bankDetails?.phone_number || bank?.account_masked}
                  </strong>
                </p>
              </div>
            </div>
