  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // ✅ Prefer withdraw keys; fall back to deposit keys or generic names
        const min = pickNumber(
          settings?.min_withdraw,
          settings?.withdraw_min,
          settings?.min, // generic
          settings?.min_deposit // fallback if backend reuses keys
        );
        const max = pickNumber(
          settings?.max_withdraw,
          settings?.withdraw_max,
          settings?.max, // generic
          settings?.max_deposit // fallback if backend reuses keys
        );

        if (!isPosFinite(min) || !isPosFinite(max) || min >= max) {
          setAmounts([]);
...
function generateFourButtons(min, max) {
  ...
  if (arr.length > 4) {
    const tt1 = min + (max - min) / 3;
    const tt2 = min + (2 * (max - min)) / 3;
    const mids = arr.slice(1, -1);
    mids.sort((a, b) => {
      const da = Math.min(Math.abs(a - tt1), Math.abs(a - tt2));
      const db = Math.min(Math.abs(b - tt1), Math.abs(b - tt2));
      return da - db;
    });
